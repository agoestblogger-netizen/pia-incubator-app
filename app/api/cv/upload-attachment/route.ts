...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!timId || !file) {
      return NextResponse.json({ success: false, message: 'Parameter tidak lengkap (timId, file dibutuhkan).' }, { status: 400 });
    }

    const allowed = await hasPermission(user, 'cust_val.edit', timId);
    if (!allowed) {
      return NextResponse.json({ success: false, message: 'Anda tidak memiliki izin mengunggah lampiran di tim ini.' }, { status: 403 });
    }
    ...
    if (!ALLOWED_EXTENSIONS.includes(ext)) {
      return NextResponse.json({
        success: false,
        message: `Format .${ext} tidak diizinkan. Tipe yang didukung: Gambar (jpg, png, webp), PDF, Word, Excel.`,
      }, { status: 400 });
    }
    ...
    if (file.size > MAX_SIZE) {
      return NextResponse.json({
        success: false,
        message: `Ukuran file melebihi batas 10MB (${(file.size / (1024 * 1024)).toFixed(2)} MB).`,
      }, { status: 400 });
    }
    ...
    const storagePath = `cv/${timId}/${Date.now()}_${sanitized}`;
